...
type VectorStore interface {
	...
	UpdateBucket(bucket v1disk.Bucket)
...
func New(params *models.Quantizer, bucket v1disk.Bucket, distFnName string, vectorLength int) (VectorStore, error) {
	// ---------------------------
	...
	if params == nil || params.Type == models.QuantizerNone {
		ps := plainStore{
			items:  meta_cache.NewItemCache[uint64, plainPoint](bucket),
			distFn: distFn,
		}
		return ps, nil
	}
