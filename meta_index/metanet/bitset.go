...
type DistSetElem struct {
	Point        dimstore.VectorStorePoint
	Distance     float32
	visited      bool
	pruneRemoved bool
}
...
type DistSet struct {
	items       []DistSetElem
	set         visitedSet
	distFn      dimstore.PointIdDistFn
	sortedUntil int
}

func NewDistSet(capacity int, maxNodeId uint64, distFn dimstore.PointIdDistFn) DistSet {
	var set visitedSet
	visitSize := maxNodeId
...
func (ds *DistSet) AddWithLimit(points ...dimstore.VectorStorePoint) {
...
func (ds *DistSet) Add(points ...dimstore.VectorStorePoint) {
	for _, p := range points {
		if ds.set.CheckAndVisit(p.Id()) {
			continue
		}
