...
func (v *IndexMetaNet) insertWorker(ctx context.Context, jobQueue <-chan IndexVectorChange) <-chan error {
	return submodules.SinkWithContext(ctx, jobQueue, func(change IndexVectorChange) error {
		return v.insertSinglePoint(change)
	})
}

func (v *IndexMetaNet) insertSinglePoint(change IndexVectorChange) error {
