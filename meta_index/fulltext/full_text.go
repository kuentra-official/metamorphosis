...
func newBeleveAnalyser(name string) (*bleveAnalyser, error) {
	analyser, err := analyserCache.AnalyzerNamed(name)
	if err != nil {
		return nil, fmt.Errorf("error getting analyser: %w", err)
	}
	return &bleveAnalyser{analyser: analyser}, nil
}

func (a *bleveAnalyser) Analyse(text string) ([]Token, error) {
...
type indexText struct {
	analyser analyser
	setCache *meta_cache.ItemCache[string, *setCacheItem]
	docCache *meta_cache.ItemCache[uint64, docCacheItem]
	numDocs  uint64
	bucket   v1disk.Bucket
	mu       sync.Mutex
}
...
// of tokens.
func NewIndexText(b v1disk.Bucket, params models.IndexTextParameters) (*indexText, error) {
	analyser, err := newBeleveAnalyser(params.Analyser)
	if err != nil {
		return nil, fmt.Errorf("error getting analyser: %w", err)
	}
	it := &indexText{
		analyser: analyser,
		setCache: meta_cache.NewItemCache[string, *setCacheItem](b),
		docCache: meta_cache.NewItemCache[uint64, docCacheItem](b),
		bucket:   b,
	}
	// ---------------------------
...
func (index *indexText) InsertUpdateDelete(ctx context.Context, in <-chan Document) <-chan error {
	...
	go func() {
		...
		sinkErrC := submodules.SinkWithContext(ctx, analysedDocs, index.processAnalysedDoc)
	...
	return submodules.MergeErrorsWithContext(ctx, errC, writeErrC)
}
...
func (index *indexText) processAnalysedDoc(ad analysedDocument) error {
	...
	if err != meta_cache.ErrNotFound && err != nil {
		return fmt.Errorf("error getting doc cache item: %w", err)
	}
	exists := err != meta_cache.ErrNotFound
	switch {
	// ---------------------------
...
func (index *indexText) parallelAnalyse(ctx context.Context, in <-chan Document) (<-chan analysedDocument, <-chan error) {
	...
	for i := 0; i < numWorkers; i++ {
		out, errC := submodules.TransformWithContext(ctx, in, func(doc Document) (ad analysedDocument, skip bool, err error) {
			// Perform analysis
	...
	return submodules.MergeWithContext(ctx, outs...), submodules.MergeErrorsWithContext(ctx, errCs...)
}
...
func (si *setCacheItem) ReadFrom(term string, bucket v1disk.Bucket) (*setCacheItem, error) {
...
func (si *setCacheItem) WriteTo(term string, bucket v1disk.Bucket) error {
	if si.set.IsEmpty() {
...
func (si *setCacheItem) DeleteFrom(term string, bucket v1disk.Bucket) error {
...
func (dc docCacheItem) ReadFrom(id uint64, bucket v1disk.Bucket) (item docCacheItem, err error) {
...
func (dc docCacheItem) WriteTo(id uint64, bucket v1disk.Bucket) error {
	if dc.Length == 0 {
...
func (dc docCacheItem) DeleteFrom(id uint64, bucket v1disk.Bucket) error {
